...
namespace AlBadrSystems\Generator\Common;
...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsRepository = config('albadrsystems.laravel_generator.namespace.repository', 'App\Repositories').$prefix;
        $this->nsModel = config('albadrsystems.laravel_generator.namespace.model', 'App\Models').$prefix;
        if (config('albadrsystems.laravel_generator.ignore_model_prefix', false)) {
            $this->nsModel = config('albadrsystems.laravel_generator.namespace.model', 'App\Models');
        }
        $this->nsSeeder = config('albadrsystems.laravel_generator.namespace.seeder', 'Database\Seeders').$prefix;
        ...
        $this->nsRequest = config('albadrsystems.laravel_generator.namespace.request', 'App\Http\Requests').$prefix;
        $this->nsRequestBase = config('albadrsystems.laravel_generator.namespace.request', 'App\Http\Requests');
        $this->nsBaseController = config('albadrsystems.laravel_generator.namespace.controller', 'App\Http\Controllers');
        $this->nsController = config('albadrsystems.laravel_generator.namespace.controller', 'App\Http\Controllers').$prefix;

        $this->nsApiTests = config('albadrsystems.laravel_generator.namespace.api_test', 'Tests\APIs');
        $this->nsRepositoryTests = config('albadrsystems.laravel_generator.namespace.repository_test', 'Tests\Repositories');
        $this->nsTests = config('albadrsystems.laravel_generator.namespace.tests', 'Tests');
    }
    ...
    public function loadPaths()
    {
        ...
        $this->pathModel = config('albadrsystems.laravel_generator.path.model', app_path('Models/')).$prefix;
        if (config('albadrsystems.laravel_generator.ignore_model_prefix', false)) {
            $this->pathModel = config('albadrsystems.laravel_generator.path.model', app_path('Models/'));
        }
        ...
        $this->pathApiRoutes = config('albadrsystems.laravel_generator.path.api_routes', base_path('routes/api.php'));
        ...
        $this->pathRequest = config('albadrsystems.laravel_generator.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('albadrsystems.laravel_generator.path.routes', base_path('routes/web.php'));
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        $commandData->addDynamicVariable(
            '$API_PREFIX$',
            config('albadrsystems.laravel_generator.api_prefix', 'api')
        );

        $commandData->addDynamicVariable(
            '$API_VERSION$',
            config('albadrsystems.laravel_generator.api_version', 'v1')
        );
    ...
    public function prepareOptions(CommandData &$commandData)
    {
        ...
        if (empty($this->options['save'])) {
            $this->options['save'] = config('albadrsystems.laravel_generator.options.save_schema_file', true);
        }

        if (empty($this->options['localized'])) {
            $this->options['localized'] = config('albadrsystems.laravel_generator.options.localized', false);
        }
        ...
        $this->options['softDelete'] = config('albadrsystems.laravel_generator.options.softDelete', false);
        $this->options['repositoryPattern'] = config('albadrsystems.laravel_generator.options.repository_pattern', true);
        $this->options['resources'] = config('albadrsystems.laravel_generator.options.resources', true);
    ...
    {
        $this->prefixes['route'] = explode('/', config('albadrsystems.laravel_generator.prefixes.route', ''));
        $this->prefixes['path'] = explode('/', config('albadrsystems.laravel_generator.prefixes.path', ''));
        $this->prefixes['view'] = explode('.', config('albadrsystems.laravel_generator.prefixes.view', ''));
        $this->prefixes['public'] = explode('/', config('albadrsystems.laravel_generator.prefixes.public', ''));
    ...
    {
        $this->addOns['swagger'] = config('albadrsystems.laravel_generator.add_on.swagger', false);
        $this->addOns['tests'] = config('albadrsystems.laravel_generator.add_on.tests', false);
        $this->addOns['datatables'] = config('albadrsystems.laravel_generator.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('albadrsystems.laravel_generator.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('albadrsystems.laravel_generator.add_on.menu.menu_file', 'layouts.menu');
    }
}
